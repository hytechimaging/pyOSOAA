...
class VSVZA(object):
    ...
    def __init__(self, resroot, filename="LUM_vsVZA.txt"):
        ...
        with open(
            os.path.join(resroot, "Standard_outputs", filename), encoding="iso-8859-15"
        ) as file:
            self.fulltext = file.readlines()

        # Read variables not tabulated
        self.vzaless = ExtractValue(self.fulltext, "for VZA < 0 (sign convention):")
        self.vzamore = ExtractValue(self.fulltext, "for VZA > 0 (sign convention):")

        # Get header length to skip it
        skipheader = [
            idx for idx, text in enumerate(self.fulltext) if "VZA    SCA_ANG" in text
        ][0]
        (
            self.vza,
            self.scaang,
            ...
            os.path.join(resroot, "Standard_outputs", filename),
...
class VSZ(object):
    ...
    def __init__(self, resroot, filename=None):
        ...
        with open(
            ...
            self.fulltext = file.readlines()
        # Read variables not tabulated
        self.simazimuth = ExtractValue(
            self.fulltext, "Simulated relative azimuth (degrees) :"
        )
        self.updirecion = ExtractValue(
            self.fulltext, "Upward direction VZA (degrees) :"
        )

        # Get header length to skip it
        skipheader = [
            idx
            for idx, text in enumerate(self.fulltext)
            if "       Z     SCA_ANG" in text
        ][0]
        (
            self.z,
            self.scaang,
...
class BIN(object):
    ...
    def __init__(self, resroot, filename="LUM_SF.bin", NT_TOT=107, NBMU=51):
        ...
        with FortranFile(
            os.path.join(resroot, "Advanced_outputs", filename), "r"
        ) as file:
            tmp = file.read_reals()
...
class ADVUPDOWN(object):
    ...
    def __init__(self, resroot, filename):
        ...
        with open(
            os.path.join(resroot, "Advanced_outputs", filename), encoding="iso-8859-15"
        ) as file:
            self.fulltext = file.readlines()
        # Read variables not tabulated
        self.vzaless = ExtractValue(self.fulltext, "for VZA < 0 (sign convention):")
        ...
        (
            ...
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
            unpack=True,
            encoding="iso-8859-15",
        )
...
class ADVPHI(object):
    ...
    def __init__(self, resroot, filename):
        ...
        # Get header length to skip it
        skipheader = [
            idx for idx, text in enumerate(self.fulltext) if "VZA    PHI" in text
        ][0]
        (
            self.vza,
            self.phi,
...
class PROFILE_SEA(object):
    ...
    def __init__(self, resroot, filename="PROFILE_SEA.txt"):
        ...
        # Get header length to skip it
        skipheader = 10
        self.level, self.z, self.tau, self.mixmol, self.mixphy, self.mixpc = (
            np.genfromtxt(
                os.path.join(resroot, "Advanced_outputs", filename),
                skip_header=skipheader + 1,
                unpack=True,
...
class PROFILE_ATM(object):
    ...
    def __init__(self, resroot, filename="PROFILE_ATM.txt"):
        ...
        # Get header length to skip it
        skipheader = 9
        self.level, self.z, self.tau, self.mixaer, self.mixray = np.genfromtxt(
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
            unpack=True,
            encoding="iso-8859-15",
...
class PM(object):
    ...
    def __init__(self, resroot, filename):
        ...
        with open(
            ...
            self.fulltext = file.readlines()
        # Read variables not tabulated
        self.extcs = ExtractValue(
            self.fulltext, "EXTINCTION CROSS SECTION (mic^2)     :"
        )
        self.scacs = ExtractValue(
            self.fulltext, "SCATTERING CROSS SECTION (mic^2)     :"
        )
        self.asymm = ExtractValue(
            self.fulltext, "ASYMMETRY FACTOR (no truncation)     :"
        )
        self.mpd = ExtractValue(self.fulltext, "MEAN PARTICULES ALTITUDE/DEPTH (m)   :")
        self.vol = ExtractValue(self.fulltext, "VOLUME OF A MEAN PARTICULE (mic^3)   :")
        self.rindex = ExtractValue(
            self.fulltext, "MEAN REFRACTIVE INDEX (real part)    :"
        )
        self.trunca = ExtractValue(
            self.fulltext, "TRUNCATION COEFFICIENT               :"
        )
        self.singlesca = ExtractValue(
            self.fulltext, "SINGLE SCATTERING ALBEDO (truncation): "
        )
        ...
        skipheader = [
            idx
            for idx, text in enumerate(self.fulltext)
            if "ALPHA(K)        BETA11(K)" in text
        ][0]
        self.alpha, self.beta, self.gamma, self.xi = np.genfromtxt(
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
            unpack=True,
            encoding="iso-8859-15",
...
class HYDIOP(object):
    """File containing the inherent optical properties (IOP) of chlorophyll or
    Mineral-Like particles
    """

    def __init__(self, resroot, filename):
        """First thirteen header lines provide comments on the content of the columns
        ...
        # First part : Phytoplankton phase matrix
        scaang      Scattering angle (degrees)
        P11phy      The coefficient P11 related to the polarized phase
                    functions of phytoplankton
        P12phy      The coefficient P12 related to the polarized phase
                    functions of phytoplankton
        P22phy      The coefficient P22 related to the polarized phase
                    functions of phytoplankton
        P33phy      The coefficient P33 related to the polarized phase
                    functions of phytoplankton

        # Second part : Mineral-like particles phase matrix
        P11nap      The coefficient P11 related to the polarized phase
                    functions of mineral particles
        P12nap      The coefficient P12 related to the polarized phase
                    functions of mineral particles
        P22nap      The coefficient P22 related to the polarized phase
                    functions of mineral particles
        P33nap      The coefficient P33 related to the polarized phase
                    functions of mineral particles

        # Third part : scattering and back-scattering profiles
        z           Depth (m)
        bphy        Scattering coefficient for phytoplankton (m-1)
        bbphy       Back-scattering coefficient for phytoplankton (m-1)
        ratiophy    Ratio of back-scattering/scattering for phytoplancton
        bnap        Scattering coefficient for mineral-like particles (m-1)
        bbnap       Back-scattering coefficient for mineral-like particles (m-1)
        rationap    Ratio of back-scattering/scattering for mineral-like particles
        b           Scattering coefficient for the global mixture of particles (m-1)
        bb          Back-scattering coefficient for the global mixture of particles (m-1)
        ratio       Ratio of back-scattering/scattering for the global mixture of particles

        These coefficients are adjusted to a phase function truncation if
        applied.
        """

        f = open(os.path.join(resroot, "Advanced_outputs", filename), "r")
        text = f.readlines()
        NbOfLine = len(text)

        # First part : Phytoplankton phase matrix
        skipheader = 12
        skipfooter = NbOfLine - 104
        self.scaang, self.P11phy, self.P12phy, self.P22phy, self.P33phy = np.genfromtxt(
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
        ...
        # Second part : Mineral-like particles phase matrix
        skipheader = 111
        skipfooter = NbOfLine - 198
        self.scaang, self.P11nap, self.P12nap, self.P22nap, self.P33nap = np.genfromtxt(
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
            skip_footer=skipfooter,
            unpack=True,
...
class AERIOP(object):
    """File containing the inherent optical properties of aerosols"""

    def __init__(self, resroot, filename):
        """First thirteen header lines provide comments and formatted data on
        the extinction and scattering cross-sections (in μm 2 ) and the
        ...
        scaang      Scattering angle (degrees)
        P11         The coefficient P11 related to the polarized phase
                    functions
        P12         The coefficient P12 related to the polarized phase
                    functions
        P22         The coefficient P22 related to the polarized phase
                    functions
        P33         The coefficient P33 related to the polarized phase
                    functions
        ...
        with open(
            ...
            self.fulltext = file.readlines()
        # Read variables not tabulated
        self.extcs = ExtractValue(self.fulltext, "Extinction cross-section (mic^2):")
        self.scacs = ExtractValue(self.fulltext, "Scattering cross-section (mic^2):")
        self.singlesca = ExtractValue(
            self.fulltext, "Single scattering albedo        :"
        )

        # Get header length to skip it
        skipheader = 14
        self.scaang, self.P11, self.P12, self.P22, self.P33 = np.genfromtxt(
            os.path.join(resroot, "Advanced_outputs", filename),
            skip_header=skipheader + 1,
            unpack=True,
            encoding="iso-8859-15",
...
class OUTPUTS(object):
    ...
    def __init__(self, resroot, filenames):
        ...
        if filenames.advup is not None:
            if os.path.exists(
                os.path.join(resroot, "Advanced_outputs", filenames.advup)
            ):
                self.advup = ADVUPDOWN(resroot, filenames.advup)

        if filenames.advdown is not None:
            if os.path.exists(
                os.path.join(resroot, "Advanced_outputs", filenames.advdown)
            ):
                self.advdown = ADVUPDOWN(resroot, filenames.advdown)

        if filenames.advphi is not None:
            if os.path.exists(
                os.path.join(resroot, "Advanced_outputs", filenames.advphi)
            ):
        ...
        if filenames.hydiop is not None:
            self.hydiop = HYDIOP(resroot, filenames.hydiop)

        if filenames.aeriop is not None:
            self.aeriop = AERIOP(resroot, filenames.aeriop)
